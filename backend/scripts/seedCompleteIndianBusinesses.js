...
// Complete realistic Indian businesses covering ALL business types
const completeIndianBusinesses = [
  // Food Mall - All Business Types
  {
    name: "Rajesh Kumar",
  ...
  // Retail / E-Commerce businesses - All Business Types
  {
    name: "Ritu Kumar",
  ...
  // Creative & Design businesses - All Business Types
  {
    name: "Print Master",
  ...
  // Agencies & Studios - All Business Types
  {
    name: "Digital Boost",
...
const seedCompleteIndianBusinesses = async () => {
  try {
    // Connect to MongoDB
    ...
    businessCategories.forEach(cat => {
      ...
      allBusinessTypes.push(...cat.businessTypes.filter(type => type.isActive).map(type => `${cat.name} - ${type.name}`));
    });
    ...
    const newBusinesses = completeIndianBusinesses.filter(b => !existingEmailSet.has(b.email));
    ...
    const categoryStats = {};
    completeIndianBusinesses.forEach(business => {

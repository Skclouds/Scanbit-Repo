...
export const ExitIntentPopup = ({ ad, onClose, onShow }: ExitIntentPopupProps) => {
  ...
  const bgStyle = ad.gradient?.enabled
    ? {
        background: `linear-gradient(135deg, ${ad.gradient.colors.join(", ")})`,
      }
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
      <div
        className="relative rounded-2xl shadow-2xl p-8 max-w-md w-full animate-in zoom-in"
        style={{
          ...bgStyle,
          color: ad.textColor,
          width: ad.displaySettings?.width || "450px",
        }}
      >
